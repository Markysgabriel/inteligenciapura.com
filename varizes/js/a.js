(function () {
    const elements = document.querySelectorAll('[class*="track-"]');
    ...
    function handleBtnClick(event, link) {
        ...
        for (let i = 0; i < classes.length; i++) {
            if (classes[i].startsWith("track-")) {
                trackId = classes[i].replace("track-", "");
                break;
            }
        }
        ...
        if (foundIndex !== -1) {
            ...
            btnClicks.push({ id: trackId, count: 1 });
        }
    ...
    if ("onbeforeunload" in window) {
        console.log("Yes onbeforeunload");
